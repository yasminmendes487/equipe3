    ...
    <div className="min-h-screen flex">
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden items-center justify-center"
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="relative z-10 text-center px-12">
          ...
          <p className="text-lg text-white/80 max-w-md">Plataforma completa para gestão de eventos, inscrições, check-in e certificados.</p>
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="w-full max-w-md">
          <div className="lg:hidden flex items-center gap-3 mb-10">
            ...
            <span className="text-xl font-bold" style={{ fontFamily: 'var(--font-display)' }}>EventFlow</span>
          </div>
          <h2 className="text-3xl font-bold mb-2" style={{ fontFamily: 'var(--font-display)' }}>Bem-vindo de volta</h2>
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input type="email" placeholder="seu@email.com"
                  className="w-full h-12 pl-10 pr-4 rounded-lg border border-input bg-background text-sm"
            ...
            <button type="submit" className="w-full h-12 bg-primary text-primary-foreground rounded-lg font-medium flex items-center justify-center gap-2 hover:bg-primary/90 transition-colors" disabled={loading}>
              {loading ? 'Entrando...' : 'Entrar'}
              {!loading && <ArrowRight className="w-4 h-4" />}
            </button>
          </form>
          <p className="text-center text-sm text-muted-foreground mt-8">
            Não tem conta?{' '}<button onClick={() => navigate('/cadastro')} className="text-primary font-medium hover:underline">Registre-se</button>
          </p>
        </motion.div>
      </div>
    </div>
