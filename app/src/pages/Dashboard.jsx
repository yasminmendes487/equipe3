...
import { mockEvents, mockEnrollments, mockCheckIns } from '@/data/mockData.js';
  ...
  return (
    <AppLayout title="Dashboard" subtitle="Visão geral da plataforma">
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5 mb-8">
        <StatCard icon={Calendar} title="Eventos Ativos" value={mockEvents.filter(e => e.status === 'aberto').length} subtitle="eventos abertos" trend={{ value: 12, positive: true }} />
        <StatCard icon={Users} title="Total Inscrições" value={mockEnrollments.length} subtitle="todas as inscrições" trend={{ value: 8, positive: true }} />
        <StatCard icon={ClipboardCheck} title="Check-ins Hoje" value={mockCheckIns.length} subtitle="realizados" />
        <StatCard icon={Award} title="Certificados" value={mockCheckIns.filter(c => c.certificateIssued).length} subtitle="emitidos" />
      </div>
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="glass rounded-2xl p-6">
          ...
          <div className="space-y-4">
            {mockEvents.filter(e => e.status === 'aberto').map((event) => (
